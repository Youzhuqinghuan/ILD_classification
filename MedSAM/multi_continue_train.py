...
parser.add_argument("--data_path", type=str, default='/jizhicfs/chengpenghu/med_datasets/data_preprocessed/npy/CT', help="Path to the dataset")
...
parser.add_argument("--output_dir", "-o", type=str, default="./outputs/task", help="Directory to save the model and logs")
...
class NpyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt2D = np.eye(num_classes)[gt]  # num_classes 是类别数量
        gt2D = np.transpose(gt2D, (2, 0, 1))
...
class MedSAM(nn.Module):
    ...
    def forward(self, image, box):
        ...
        # 调整输出形状，使其与 gt2D 匹配
        ori_res_masks = F.softmax(ori_res_masks, dim=1)
        ori_res_masks = ori_res_masks.expand(-1, self.num_classes, -1, -1)
...
def main():
    ...
    # 修改损失函数
    seg_loss = monai.losses.DiceLoss(squared_pred=True, reduction='mean')
        ...
        for step, (image, gt2D, boxes, _) in enumerate(tqdm(train_dataloader)):
            ...
            if args.use_amp:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    loss = alpha * seg_loss(medsam_pred, gt2D) + beta * ce_loss(medsam_pred, gt2D.argmax(dim=1))
